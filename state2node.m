...
%     end
    ...
    frenet_angle_dist_weight=zeros(states_count,1);
...
angle_cost_on_straight = 2*max(0,(1-10*abs(states(:,7)))).*normalize_vector(states(:,3).^2);
     
cost2go_weight = .6; %.4;     
dyn_states_coeffs = 1.05*[1.5 5*1 6 5*.5 1 0]; 
...
avoid_edges =2*10*normalize_vector(max(0,states(:,2).^4-4^4));
keep_centre = 2*5*normalize_vector(states(:,2).^2);
